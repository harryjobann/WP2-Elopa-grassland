...
# tagging data
exifdat_test_05 <- read.csv('https://raw.githubusercontent.com/harryjobann/WP2-Elopa-grassland/refs/heads/main/exifdat_test_05.csv', 
                           header = TRUE, colClasses = 'character')
# deployments
deptab_test_05 <- read.csv('https://raw.githubusercontent.com/harryjobann/WP2-Elopa-grassland/refs/heads/main/deptab_test_05.csv', header = TRUE)

# animal position data
posdat_dibang <- read.csv('https://raw.githubusercontent.com/harryjobann/WP2-Elopa-grassland/refs/heads/main/posdat_dibang.csv', header = TRUE)

# speed data (aggregated acros both seasons of Grassland data (2021 & 2024 to improve data integrity and robustness))
seqdat_dibang <- read.csv('https://raw.githubusercontent.com/harryjobann/WP2-Elopa-grassland/refs/heads/main/combined_seqdat.csv', header = TRUE)
...
# rename cols
seqdat_dibang$station <- seqdat_dibang$Location.ID
posdat_dibang$station <- posdat_dibang$deployment
...
# Subsetting for records where there was an animal contact
exifdat_test_05$station <- sub(" ", "", exifdat_test_05$station)
contactdat_test_05 <- subset(exifdat_test_05, contact==1)

# Check which stations included
unique(contactdat_test_05$station)
...
# Convert 'start' and 'stop' columns in deptab_test_05 to desired format
...
contactdat_test_05$date <- as.POSIXct(contactdat_test_05$date, format = "%Y-%m-%d %H:%M:%S", tz = "UTC")
...
# Change year from 2020 to 2024 for the few records this applies to
contactdat_test_05 <- dplyr::mutate(
  contactdat_test_05,
  date = dplyr::case_when(
    format(as.Date(date), "%Y") == "2020" ~ as.POSIXct(sub("^2020", "2024", date)),
    TRUE ~ as.POSIXct(date)
  )
)

#Update deptab start/stop values to include contacts currently being excluded.
deptab_test_05$stop[21] <- as.POSIXct("2024-04-06 19:00:00")
deptab_test_05$start[5] <- as.POSIXct("2024-02-10 13:00:00")
...
#rename
contactdat_test_05$number.of.individuals <- contactdat_test_05$number_of_individuals


# Step 1: Filter the dataframe to keep only rows where number.of.individuals starts with "c-"
contactdat_test_05 <- dplyr::filter(contactdat_test_05, grepl("^c-", number.of.individuals))
...
species_list <- c(
                 "Hog_deer", "Sambar", "Indian_jackal", "Asiatic_wild_dog", "Small_indian_civet",  "Wild_boar",  "Jungle_cat" , "Large_indian_civet",  "Malayan_porcupine", "Yellow_throated_marten",
                  "Indian_hare"
               # , "Buffalo", "Cow"
             )


# Finally, create a dataframe of trap rate data using event.count. 
...
# This block of code is to test the REM calculation is functioniting correctly or to test any alterations in the script

# rename
contactdat_test_05$time <- contactdat_test_05$radiantime

# define species to test
sp <- "Indian_hare"


# Speed
speeds <- subset(seqdat_dibang, species==sp )$speed
hist(log10(speeds), main="", xlab="Speed (log10[m/s])")
speeds <- subset(seqdat_dibang, species==sp & speed>0.001 & speed<10)$speed
spdest <- hmean(speeds)


# Activity
actmod <- fitact(subset(contactdat_test_05, species==sp)$time,
...
# Radius
dzdat <- subset(posdat_dibang, frame_count==1 & species==sp
          #      & radius <= 10
                )

mean(dzdat$radius)
radmod <- fitdf(radius ~ 1, dzdat, transect = "point", order=0, key = "hr", truncation = 15)
radmod$edd
plot(radmod$ddf, pdf=TRUE)

# Angle
dzdat$angle <- abs(dzdat$angle)
angmod <- fitdf(angle~1, dzdat, key="hr", order=0)
angmod$edd
plot(angmod$ddf)
...
radius_model_selection <- list(
  "Hog_deer" = "hn",
  "Malayan_porcupine" = "hr"
)

angle_model_selection <- list(
  "Small_indian_civet" = "hr",
  "Yellow_throated_marten" = "hr",
  "Malayan_porcupine" = "hr"
)
...
for (sp in species_list) {
  ...
  for (model in radius_models) {
    ...
    # Fit the model
    radmod <- tryCatch({
      fitdf(radius ~ 1, dzdat_elopa_5_dibang, transect = "point", order=0, key = model, truncation = 20)
    }, error = function(e) {
...
# write.csv(new_info_final, file = "/Users/harryjobanputra/Documents/ZSL - Research assistant/WP2 - Grassland Elopa/Handover_Grassland/Results/2024_density_results", row.names = FALSE)
